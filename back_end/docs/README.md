...
- **Single-Document Model**: one document per language per entity — see "Core Concepts" below.
...
- `phrase_index` - Recurring-phrase index for `get_phrase_context`
...
- [Bible Import](./import.md) - USFM import
- [OpenRouter Integration](./openrouter_docs.md) - OpenRouter-specific API details (thinking, caching, provider routing)
...
See [configuration.md](./configuration.md#local-development-setup) for starting the bundled `mongod` and verifying the connection.