...
def _ensure_mongod():
    ...
    if not _port_open():
        ...
        proc = subprocess.Popen(
            [str(mongod_bin), "--port", "27019", "--dbpath", str(db_path), "--bind_ip", "127.0.0.1", "--auth"],
    ...
    # Runs on both branches: a hand-started or Electron-started mongod that
    # skipped the spawn above must be enforcing auth too, or the whole suite
    # would silently run against a downgraded database.
    try:
        verify_auth_enforced()
    except AuthEnforcementError as e:
        pytest.fail(str(e))

    yield

    # Teardown: only if we spawned it.
    if proc is not None:
        proc.terminate()
        try:
